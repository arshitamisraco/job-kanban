...
## Needs Arshita
- Nothing blocking. Two things worth a look on a real Mac: SF Pro rendering (screenshots here used the Linux fallback font) and Safari's backdrop-filter on the sheet (tested in headless Chromium only).
- `docs/screenshot.png` was regenerated by the smoke test; swap in a Mac screenshot if you want SF Pro in the README.
